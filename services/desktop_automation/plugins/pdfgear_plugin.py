...
class PDFGearPlugin(BaseAutomationPlugin):
    ...
    def execute(self, request: ActionRequest, backend) -> ActionResult:
        ...
        try:
            ...
            # 2. 🎯 MÉTODO HÍBRIDO: Visión por computadora + Fallback
                    ...
                    for confidence in [0.9, 0.8, 0.7, 0.6, 0.5]:
                        try:
                            ...
                            if button_location:
                                ...
                                logger.info(
                                    f"[PDFGear] ✅ Botón encontrado con confianza {confidence} en X={btn_x}, Y={btn_y}")
            ...
            if not button_clicked:
                logger.warning("[PDFGear] ️ Visión por computadora falló. Usando coordenadas relativas...")

                btn_x = win.left + int(win.width * 0.50)
                btn_y = win.top + int(win.height * 0.13)
            ...
            # 3. Verificar que se abrió el diálogo de Windows
            ...
            # 4. 🎯 INYECTAR RUTA USANDO PYWINAUTO
            ...
            # 5. ⏳ ESPERAR A QUE PDFGEAR CARGUE EL ARCHIVO
            ...
            if os.path.exists(btn_dots_img):
                ...
                try:
                    ...
                    logger.warning(f"[PDFGear] ⚠️ No se encontró botón '...' con computer vision: {e}")

            if not btn_dots_clicked:
                logger.warning("[PDFGear] ⚠️ Usando coordenadas para botón '...'")
                btn_dots_x = win.left + int(win.width * 0.95)
                btn_dots_y = win.top + int(win.height * 0.85)
                pyautogui.click(btn_dots_x, btn_dots_y)

            # ⏳ ESPERAR MÁS TIEMPO para que el diálogo "Seleccionar carpeta" se abra
            logger.info("[PDFGear]  Esperando a que se abra el diálogo 'Seleccionar carpeta'...")
            time.sleep(4)

            # Buscar el diálogo "Seleccionar carpeta"
            logger.info("[PDFGear] 🔍 Buscando diálogo 'Seleccionar carpeta'...")

            folder_dialog_found = False
            folder_dialog = None

            for attempt in range(10):  # Más intentos
                all_wins = gw.getAllWindows()
                for w in all_wins:
                    if w.visible and not w.isMinimized:
                        title_lower = w.title.lower()
                        # Buscar "seleccionar carpeta" específicamente
            ...
            if folder_dialog_found and folder_dialog:
                ...
                try:
                    ...
                    # Imprimir controles para depuración
                    logger.info("[PDFGear] 🔍 Controles del diálogo:")
                    dlg_folder.print_control_identifiers()

                    # Buscar el campo "Carpeta:" (Edit control en la parte inferior)
                    # Basado en la estructura típica, buscar por título o auto_id
                    folder_edit = None

                    # Método 1: Buscar por título "Carpeta:"
                    try:
                    ...
                    if not folder_edit:
                        try:
                            folder_edit = dlg_folder.child_window(auto_id="1001", control_type="Edit")
                            folder_edit.wait('ready', timeout=3)
                            logger.info("[PDFGear] ✅ Campo 'Carpeta:' encontrado por auto_id")
                        except:
                            pass

                    # Método 3: Buscar el último Edit control (suele ser el campo de carpeta)
                    if not folder_edit:
                        try:
                            all_edits = dlg_folder.descendants(control_type="Edit")
                            if all_edits:
                                folder_edit = all_edits[-1]  # Tomar el último
                    ...
                    if folder_edit:
                        ...
                        try:
                            ...
                            # Intentar con auto_id
                            try:
                                btn_select = dlg_folder.child_window(auto_id="1", control_type="Button")
                                btn_select.click_input()
                                logger.info("[PDFGear] ✅ Clic en botón por auto_id")
                            except Exception as e:
                                logger.warning(f"[PDFGear] ⚠️ No se pudo hacer clic en 'Seleccionar carpeta': {e}")

                        time.sleep(2)
                        logger.info("[PDFGear] ✅ Ruta de salida cambiada exitosamente")
                    else:
                        logger.warning("[PDFGear] ⚠️ No se encontró el campo 'Carpeta:'")

                except Exception as e:
                    logger.error(f"[PDFGear] ❌ Error con pywinauto: {e}", exc_info=True)
            else:
                logger.warning(
                    "[PDFGear] ️ No se encontró diálogo 'Seleccionar carpeta'. Continuando con ruta por defecto...")

            # 7. 🎯 HACER CLIC EN "CONVERTIR"
            ...
            # 8. ⏳ ESPERAR A QUE FINALICE LA CONVERSIÓN
            logger.info("[PDFGear] 8. Esperando a que finalice la conversión...")
            time.sleep(20)

            # 9. 🔍 VERIFICACIÓN REAL - Buscar en múltiples ubicaciones
            ...
            if found_output:
                ...
                return ActionResult(session_id, True, "Conversión completada exitosamente",
