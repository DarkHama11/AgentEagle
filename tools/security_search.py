...
class SourceTier(Enum):
    ...
    TIER_1 = "tier1"  # Máxima confianza: fuentes oficiales
    TIER_2 = "tier2"  # Alta confianza: proveedores cloud
    TIER_3 = "tier3"  # Confianza media: blogs especializados
    TIER_4 = "tier4"  # Baja confianza: fuentes generales
    UNKNOWN = "unknown"
...
SOURCE_TIERS: Dict[str, SourceTier] = {
    # Tier 1: Fuentes oficiales de máxima confianza
    ...
    # Tier 2: Proveedores cloud oficiales
    ...
    # Tier 3: Blogs y medios especializados en seguridad
...
class SecuritySearch:
    ...
    def normalize_query(query: str) -> str:
        ...
        - elimina caracteres irrelevantes
        - ordena palabras para consistencia
        """
        # Lowercase y strip
        ...
        # Eliminar caracteres irrelevantes (mantener alfanuméricos, espacios, guiones)
        normalized = re.sub(r'[^\w\s\-]', ' ', normalized)

        # Eliminar espacios extra y colapsar
        normalized = re.sub(r'\s+', ' ', normalized).strip()

        # Ordenar palabras para consistencia (determinismo)
    ...
    def _enrich_query(self, query: str, category: SearchCategory) -> str:
        ...
        # Normalizar "security command center" para GCP
    ...
    def search(
            ...
            query: str,
            num_results: int = 5,
            category: Optional[Union[str, SearchCategory]] = None
    ) -> SearchResponse:
        """
        ...
        Args:
            ...
            category: Categoría como string ('AWS') o SearchCategory enum

        Returns:
            SearchResponse: Respuesta estructurada con resultados rankeados
        """
        start_time = time.time()

        if not self._initialized:
            return SearchResponse(
                query_original=query, query_enriched=query, category="unknown",
                results=[], confidence=0.0, total_results=0,
                execution_time_ms=0, cache_hit=False
            )

        # Manejar categoría: string, SearchCategory, o None
        if category is None:
        ...
        # ✅ CACHE KEY DETERMINISTA: solo normalized_query + category
        ...
        # Ejecutar búsqueda si no hay cache
        if not results_raw:
        ...
        # Procesar resultados: scoring y ranking
        ...
        return SearchResponse(
            ...
            query_enriched=self._enrich_query(query, search_category),
            category=search_category.name.lower(),
